...
import type { User, Exam, Question } from '../types';
...
const ExamEditorModal: React.FC<ExamEditorModalProps> = ({ exam, allStudents, onClose, onSave, onDelete }) => {
    const [formData, setFormData] = useState<Exam>(exam || {
        id: '', title: '', dateTime: '', questions: [], isEnabled: false, allowedStudentIds: [], class: '', description: ''
    });
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        setFormData(exam || {
            id: `exam_${Date.now()}`, title: '', dateTime: new Date().toISOString().substring(0, 16),
            questions: [], isEnabled: false, allowedStudentIds: [], class: '', description: ''
        });
    }, [exam]);
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {students.map(student => (
                                     <div key={student.id} onClick={() => handleOpenStudentModal(student)} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all cursor-pointer">
